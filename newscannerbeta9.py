...
class Application(QWidget):
    ...
    def setup_left_panel(self):
        ...
        project_layout.addWidget(self.forge_clean_button)
        ...
        project_layout.addWidget(self.npm_install_button)
        ...
        project_layout.addWidget(self.forge_install_button)
        ...
        project_layout.addWidget(self.run_slither_scan_button)
        
        self.run_metrics_button = QPushButton("Analyze Code Metrics (solidity-code-metrics)")
        self.run_metrics_button.setToolTip("Runs `solidity-code-metrics` on the selected project.")
        self.run_metrics_button.clicked.connect(self.run_solidity_metrics)
        project_layout.addWidget(self.run_metrics_button)

        self.run_cloc_button = QPushButton("Count Lines of Code (cloc)")
        self.run_cloc_button.setToolTip("Runs `cloc` on the selected project.")
        self.run_cloc_button.clicked.connect(self.run_cloc)
        project_layout.addWidget(self.run_cloc_button)
        ...
        self.run_coverage_button = QPushButton("Generate Coverage Report")
        self.run_coverage_button.setToolTip("Runs `forge coverage` to generate a test coverage report.")
        self.run_coverage_button.clicked.connect(self.run_forge_coverage)
        forge_layout.addWidget(self.run_coverage_button)
    ...
    def _generate_contract_map_thread(self, target_path):
        try:
            ...
            slither_instance = Slither(target_path)
    ...
    def run_command(self, command, cwd=None):
        def target():
            nonlocal command
            remappable_forge_commands = ["forge build", "forge test", "forge run", "forge script", "forge debug"]
            
            if any(command.strip().startswith(cmd) for cmd in remappable_forge_commands):
                if cwd and not os.path.exists(os.path.join(cwd, 'remappings.txt')):
                    remappings = []
                    
                    # Handle project source
                    if os.path.isdir(os.path.join(cwd, 'contracts')):
                        remappings.append(f"contracts/={os.path.join(cwd, 'contracts')}/")
                    
                    # Handle node_modules (Hardhat style)
                    node_modules_dir = os.path.join(cwd, 'node_modules')
                    if os.path.isdir(node_modules_dir):
                        remappings.append(f"@openzeppelin/contracts/=node_modules/@openzeppelin/contracts/")
                    
                    # Handle lib (Forge style)
                    lib_dir = os.path.join(cwd, 'lib')
                    if os.path.isdir(lib_dir):
                        if os.path.isdir(os.path.join(lib_dir, 'forge-std')):
                            remappings.append(f"forge-std/=lib/forge-std/src/")
                    
                    if remappings:
                        self.terminal_queue.put(f"<b>Note:</b> Auto-applying remappings for forge command.")
                        remap_flags = " ".join([f"--remappings {shlex.quote(r)}" for r in remappings])
                        command = f"{command} {remap_flags}"
            try:
    ...
    def update_exploit_params(self, finding_data=None):
        ...
        def _update_function_list(contract_name):
            """Populates the generic function dropdown based on the selected contract."""
        ...
        # --- Initial Population and Pre-selection ---
        # Pre-select contract from the finding
        ...
        # Pre-select function from the finding in our new generic dropdown
        ...
        # Populate template-specific fields AFTER setting the contract
    ...
    def run_solidity_metrics(self):
        ...
        # FIX: Pass a glob pattern to find all .sol files recursively.
        # The quotes are important for shells to handle paths with spaces correctly.
        glob_pattern = f'"{scan_path}/**/*.sol"'
        
        # Use -b to avoid box characters that might not render well in HTML.
    ...
    def run_cloc(self):
        ...
        # FIX: Add --exclude-dir to ignore dependency folders for a more accurate count.
    ...
    def run_forge_coverage(self):
        project_root = self.get_project_root()
        if not project_root:
            self.terminal_queue.put("<span style='color: red;'>No project context. Please select a Forge project folder.</span>")
            return
        self.clear_output()
        self.terminal_queue.put(f"<b>Generating test coverage report...</b>\n")
